...
async function createConsentRouter() {
...
async function mountConsentView() {
  const router = await createConsentRouter()
  const wrapper = mount(ConsentView, { global: { plugins: [router] } })
  await flushPromises()
  await flushPromises()
  return wrapper
}
