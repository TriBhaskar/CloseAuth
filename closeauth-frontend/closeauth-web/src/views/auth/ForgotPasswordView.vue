...
async function handleSubmit(): Promise<void> {
  ...
  try {
    ...
    const ok = await requestPasswordReset({ email: email.value, clientId: clientId.value || undefined })
    if (ok) {
      isSent.value = true
    } else {
  ...
  <AuthLayout>
    ...
    <div v-if="isSent" class="flex flex-col gap-4 text-center">
      <p class="text-sm text-foreground">
        If that email has an account, we've sent a password reset link to it. Check your inbox.
      </p>
      <RouterLink to="/login">
