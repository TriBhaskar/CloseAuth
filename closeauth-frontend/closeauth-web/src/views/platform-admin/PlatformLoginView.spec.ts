...
function stubLoginFetch(handler: (init: RequestInit) => { ok: boolean; status: number; body: unknown }) {
  ...
  vi.stubGlobal(
    ...
    vi.fn((url: string, init?: RequestInit) => {
      if (url === '/platform/api/login') {
        ...
        return Promise.resolve({ ok, status, json: () => Promise.resolve(body), clone() { return this } })
      }
...
describe('PlatformLoginView', () => {
  it('successful login pushes to returnTo, client-side (no window.location.assign)', async () => {
    stubLoginFetch(() => ({
      ok: true,
      status: 200,
      body: { authenticated: true, adminId: 'admin-1', email: 'staff@closeauth.test', roles: ['PLATFORM_ADMIN'], accessTokenExpiresAt: '2026-01-01T00:05:00Z' },
    }))
    const assignSpy = vi.fn()
    Object.defineProperty(window, 'location', { configurable: true, value: { assign: assignSpy } })

    const router = await createLoginRouter('/platform/login?returnTo=%2Fplatform%2Fconsole%2Ftenants')
  ...
  it('invalid credentials: uniform, enumeration-safe message, no navigation', async () => {
    stubLoginFetch(() => ({ ok: false, status: 401, body: { error: 'invalid_credentials', error_description: 'Invalid credentials.' } }))
  ...
  it('not_platform_admin: its own specific copy, not the generic invalid-credentials message', async () => {
    stubLoginFetch(() => ({ ok: false, status: 403, body: { error: 'not_platform_admin', error_description: 'refused' } }))
    ...
    const alert = wrapper.find('[role="alert"]')
    expect(alert.exists()).toBe(true)
    expect(alert.text()).toContain('does not hold PLATFORM_ADMIN')
    expect(alert.text()).not.toContain('Incorrect email or password')
  })
})
